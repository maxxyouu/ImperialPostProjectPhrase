...
class BaseCAM:
    ...
    def forward(self,
        ...
        if feature_maps_only:
            return cam_per_layer, outputs
        
        if retain_model_output:
            return self.aggregate_multi_layers(cam_per_layer, upsample=upsample), outputs
        return self.aggregate_multi_layers(cam_per_layer, upsample=upsample)
    ...
    def compute_cam_per_layer(
        ...
        for i in range(len(self.target_layers)):
            ...
            cam = self.get_cam_image(input_tensor,
                                     ...
                                     feature_maps_only,
                                     eigen_smooth)
            # for axiom evaluation case only
            if feature_maps_only:
            ...
            if upsample:
                cam = scale_cam_image(cam, target_size)
                cam_per_target_layer.append(cam[:, None, :])

        return cam_per_target_layer

    def aggregate_multi_layers(self, cam_per_target_layer: np.ndarray, upsample=True) -> np.ndarray:
